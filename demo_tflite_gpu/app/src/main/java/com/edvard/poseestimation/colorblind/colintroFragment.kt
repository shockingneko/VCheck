...
class colintroFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val root = inflater.inflate(R.layout.fragment_colintro, container, false)
        ...
        button1.setOnClickListener {
            val intent = Intent()
            intent.setClass(activity,colorblindActivity::class.java)
            startActivity(intent)
        }

        back.setOnClickListener {
            val trans = fragmentManager.beginTransaction()
            val colstart = colstartFragment()
            trans.replace(R.id.navi_container, colstart)
            trans.commit()
        }
